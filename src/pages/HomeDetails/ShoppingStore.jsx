...
import { minusItem, plusItem, removeItem } from '../../Store/index';
    ...
    <ul className='list-group'>
     {
      (card.length>0)&&(
        card.map((item,index)=>(
          ...
          <h3 >{item.title || item.name} {item.release_date}</h3>
          <p >{item.overview}</p>
          </div>
          <div className='col-md-1 d-flex align-items-center '>
          <button onClick={()=>minusItemAdd(item.id)} className="btn btn-secondary buttonItem d-block mx-2  ">
            <i className="fa-solid fa-minus "></i>
          </button>
          <span className='h4'>{item.count}</span>
          <button onClick={()=>plusItemAdd(item.id)} className="btn btn-secondary buttonItem d-block mx-2 ">
            <i className="fa-solid  fa-plus"></i>
          </button>
    
          </div>
          <div className='col-md-1 align-items-center d-flex mx-5'>
